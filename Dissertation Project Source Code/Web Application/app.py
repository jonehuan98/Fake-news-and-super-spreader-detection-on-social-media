...
# function that uses a loaded,trained svm model classifier for text prediction       
def predictions(text):
    text = inputformat(text)
    text = [text]
    model = load("model.joblib")
    # vectorises the text for feature extraction
    vectorizer = pickle.load(open("vectorizer.pickle","rb"))
    xv_test = vectorizer.transform(text)
    predicted = model.predict(xv_test)
    return predicted

# function that returns the result description of a news tweet based on its prediction
...
# function for preprocessing text input 
...
# function for obtaining the tweetid and text from a tweet link
def getTweet(link):
    url = str(link)
    tweetid = url.split('/')[-1]
...
# function for tweet parameter data collection
def getValues(link):
    url = str(link)
    tweetid = url.split('/')[-1]
    tweet1 = api.get_status(tweetid, tweet_mode='extended')
...
# function for obtaining the most recent tweets of a twitter user
...
# function for obtaining tweet replies
...
# function for obtaining super spreader score
...
# returns the embedded html code of a tweet
def getembed(link):
